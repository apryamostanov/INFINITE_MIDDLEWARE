...
class T_soap2rest_template_converter extends T_middleware_base_6_util implements I_conversion_module {
    ...
    I_http_message convert_http_messages(Map<String, I_http_message> i_messages_by_name, String i_primary_service_name) {
        ...
        l_result_http_message.set_payload(get_template_manager().get_json_template(i_primary_service_name).make(i_messages_by_name).toString())
        l_result_http_message.set_header(GC_HEADER_NAME_CONTENT_TYPE, app_conf().GC_CONTENT_TYPE_INTERNAL)
        l_result_http_message.set_header(GC_SOAPACTION_HEADER_NAME, app_conf().GC_SOAPACTION_HEADER_PREFIX + i_primary_service_name)
